...
% Cluster Prominence: [2]                   (out.Cluster_prominence)
% Cluster Shade: [2]                        (out.Cluster_shade)
% Dissimilarity: [2]                        (out.Dissimilarity)
% Energy: matlab / [1,2]                    (out.Energy)
% Entropy: [2]                              (out.Entropy)
% Homogeneity: matlab                       (out.Homogeneity)
% Homogeneity: [2]                          (out.Homogeneity_e)
% Maximum probability: [2]                  (out.Maximum_probability)
% Sum of sqaures: Variance [1]              (out.Sum_of_sqaures)
% Sum average [1]                           (out.Sum_average)
% Sum variance [1]                          (out.Sum_variance)
% Sum entropy [1]                           (out.Sum_entropy)
% Difference variance [1]                   (out.Difference_variance)
% Difference entropy [1]                    (out.Difference_entropy)
% Information measure of correlation1 [1]   (out.Information_measure_of_correlation_1)
% Informaiton measure of correlation2 [1]   (out.Information_measure_of_correlation_2)
% Inverse difference (INV) is Homogeneity [3]     (out.Homogeneity)
% Inverse difference normalized (INN) [3]   (out.Inverse_difference_normalized) 
% Inverse difference moment normalized [3]  (out.Inverse_difference_moment_normalized)
%
...
% checked
% out = struct('Autocorrelation', [], 'Contrast', [], 'Correlation', [], 'Correlation_e', [], ...
...
for k = 1:(size_glcm_3)
    ...
    for i = 1:(2*(size_glcm_1)-1)
        out.Sum_average(k) = out.Sum_average(k) + (i+1)*p_xplusy(i,k);
        % the summation for Sum_average is for i from 2 to 2*Ng hence (i+1)
...
% http://murphylab.web.cmu.edu/publications/boland/boland_node26.html
% we have for Difference_variance
...
for k = 1:size_glcm_3
    for i = 1:size_glcm_1
        for j = 1:size_glcm_2
            ...
            out.Cluster_prominence(k) = out.Cluster_prominence(k) + (((i + j - u_x(k) - u_y(k))^4)*...
                glcm(i,j,k));
            out.Cluster_shade(k) = out.Cluster_shade(k) + (((i + j - u_x(k) - u_y(k))^3)*...
                glcm(i,j,k));
        end
    end
